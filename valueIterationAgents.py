...
class PrioritizedSweepingValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                max_q_value = max([self.computeQValueFromValues(state, action) for action in self.mdp.getPossibleActions(state)], default=0)
        ...
        for i in range(self.iterations):
            ...
            for pred in predecessors.get(state, []):
                max_q_value_pred = max([self.computeQValueFromValues(pred, action) for action in self.mdp.getPossibleActions(pred)], default=0)
